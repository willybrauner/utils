...
>(
  ...
  for (let name of event) obj?.addEventListener(name, callback as EventListener, options)
  return () => {
    for (let name of event) obj?.removeEventListener(name, callback as EventListener, options)
  }
}
